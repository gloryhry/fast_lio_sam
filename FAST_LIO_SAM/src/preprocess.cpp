...
{
  // ROS_INFO_STREAM("mid360_handler");
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();
  ...
  if (feature_enabled)
  {
    for(uint i=1; i<plsize; i++)
    {
      if((pl_orig.points[i].line < N_SCANS) && ((pl_orig.points[i].tag & 0x30) == 0x10 || (pl_orig.points[i].tag & 0x30) == 0x00))
      {
        ...
        pl_full[i].curvature = (pl_orig.points[i].timestamp - pl_orig.points[0].timestamp) / float(1000000); //use curvature as time of each laser points
        // std::cout << pl_full[i].curvature << std::endl;
    ...
    for(uint i=1; i<plsize; i++)
    {
      if((pl_orig.points[i].line < N_SCANS) && ((pl_orig.points[i].tag & 0x30) == 0x10 || (pl_orig.points[i].tag & 0x30) == 0x00))
      {
        ...
        if (valid_num % point_filter_num == 0)
        {
          ...
          pl_full[i].z = pl_orig.points[i].z;
          pl_full[i].intensity = pl_orig.points[i].intensity;
          pl_full[i].curvature = (pl_orig.points[i].timestamp - pl_orig.points[0].timestamp) / float(1000000); // use curvature as time of each laser points, curvature unit: ms
          // std::cout << pl_full[i].curvature << std::endl;
...
//             added_pt.z = pl_orig.points[i].z;
//             added_pt.intensity = pl_orig.points[i].intensity;
//             added_pt.curvature = pl_orig.points[i].timestamp / 1000.0;  // curvature unit: ms
...
void Preprocess::rs_handler(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
  ...
  if (feature_enabled)
  {
    ...
    for (uint i = 0; i < plsize; i++)
    {
      ...
      if( pl_orig.points[i].x <  0.5 && pl_orig.points[i].x > -0.5 && pl_orig.points[i].y < 0.4 && pl_orig.points[i].y > -0.4) continue;
      Eigen::Vector3d pt_vec;
      PointType added_pt;
      added_pt.x = pl_orig.points[i].x;
      added_pt.y = pl_orig.points[i].y;
      ...
      added_pt.curvature = pl_orig.points[i].timestamp - msg_time;
      // if(i % 100 == 0)
